...
While I have good intentions here, I am using this project to learn Go. As such, and given my track record with large projects, it
is probable that this project will never reach a usable state.
...
go run .   # or go build .
'''
This will create the `bundled.go` file in the `.../internal/jsdrgui` directory and build or build and run the `jsdr` app. 
...
`.../internal/jsdrgui/start_stop_toolbar_action.go`.
...
- I use VSCode and some VSCode-related files are included in the jsdr repository, so you may
