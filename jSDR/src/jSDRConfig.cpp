...
void jSDRConfig::setDefaultDisplayValues()
{
    ...
    m_values["mainFrame"]["size"]["width"] = 
        wxDefaultSize.GetWidth();
    m_values["mainFrame"]["size"]["height"] =
        wxDefaultSize.GetHeight();
}

void jSDRConfig::setDefaultConfigValues() {
    setDefaultDisplayValues();
}

std::shared_ptr<jSDRConfig::DisplayProperties> 
        jSDRConfig::getDisplayProperties() {
    auto displayProps = std::make_shared<jSDRConfig::DisplayProperties>();
    displayProps->displayNumber = 
        m_values["mainFrame"]["displayNumber"].asInt();
